...
class Echo2(protocol.Protocol):
    ...
    def dataReceived(self, data: bytes) -> None:
        # Example of downloading set size file
        buffer = "hello wald".encode("utf-8")

        header = Headers()
        header.add("Content-Length", len(buffer))
        header.add("Content-Type", "application/octet-stream")

        resp = HttpResponse(status_code=STATUS_CODE_ENUM["OK"], headers=header, content=buffer)
        self.transport.write(resp.get_bytes())
    """
    def dataReceived(self, data: bytes) -> None:
        # Example of downloading chunks of data with unknown total size
        try:
            # Set up response headers
            header = Headers()
            header.add("Content-Type", "application/octet-stream")
            header.add("Transfer-Encoding", "chunked")
            header.add("Connection", "keep-alive")  # Explicitly keep connection open
            
            # Send response headers
            resp = HttpResponse(status_code=STATUS_CODE_ENUM["OK"], headers=header, content="")
            self.transport.write(resp.get_bytes())
            
            # Stream the file in chunks
            with open("./video.mp4", "rb") as file:
                chunk_size = 1024  # Larger chunks are more efficient
                while chunk := file.read(chunk_size):
                    # Send chunk size in hex format followed by CRLF
                    size = f"{len(chunk):x}\r\n".encode('utf-8')
                    # Send chunk followed by CRLF
                    self.transport.write(size + chunk + b"\r\n")
                    time.sleep(0.01)  # Small delay to prevent overwhelming the connection
                    
            # Send the final chunk to indicate end of response
